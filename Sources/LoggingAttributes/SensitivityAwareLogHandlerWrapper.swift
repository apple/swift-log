...
public struct SensitivityAwareLogHandlerWrapper: LogHandler {
    ...
    public func log(event: LogEvent) {
        ...
        // Redact sensitive values and strip their sensitivity attribute — the redacted value
        // no longer contains sensitive data, so the attribute should not propagate downstream.
        ...
        for (key, attributedValue) in merged {
            if attributedValue.attributes.sensitivity == .sensitive {
                ...
                redacted.attributes[Logger.Sensitivity.self] = nil
    ...
    public subscript(attributedMetadataKey key: String) -> Logger.AttributedMetadataValue? {
        get {
            self._attributedMetadata[key]
        }
        set {
            self._attributedMetadata[key] = newValue
        }
    }
}
