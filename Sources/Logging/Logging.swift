...
extension Logger {
    ...
    /// for the key type, so attributes occupy minimal space (one inline slot without heap
    /// allocation for the common single-attribute case). Attributes that need to carry
    /// associated data, strings, or richer payloads are outside the scope of this protocol.
    ///
    /// ## Example
    ///
    /// ```swift
    /// public enum Priority: Int, Sendable, MetadataAttributeKey {
    ///     case low = 1
    ///     case high = 2
    /// }
    /// ```
    public protocol MetadataAttributeKey: Sendable, RawRepresentable where RawValue == Int {}
    ...
    internal struct MetadataAttributeEntry: Sendable, Equatable {
        ...
        @inlinable
        internal init(key: ObjectIdentifier, value: Int) {
            self.key = key
            self.value = value
        }
    }
                ...
                if let inline = self._inline, inline.key == id {
                    if let v = newValue {
                        ...
                        if var overflow = self._overflow, !overflow.isEmpty {
                            self._inline = overflow.removeFirst()
