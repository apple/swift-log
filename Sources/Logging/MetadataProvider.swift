...
extension Logger {
    ...
    ///
    /// ```swift
    /// let provider = Logger.MetadataProvider.attributed {
    ///     [
    ...
    public struct MetadataProvider: _SwiftLogSendable {
        ...
        /// - Returns: A new metadata provider that returns attributed metadata.
        public static func attributed(
            _ provideMetadata: @escaping @Sendable () -> AttributedMetadata
        ) -> MetadataProvider {
            MetadataProvider(storage: .attributed(provideMetadata))
        }
        ...
        public func getAttributedMetadata() -> AttributedMetadata {
            switch self.storage {
            case .plain(let provide):
        ...
        return Logger.MetadataProvider(
            storage: .attributed({
                providers.reduce(into: Logger.AttributedMetadata()) { merged, provider in
                    let provided = provider.getAttributedMetadata()
