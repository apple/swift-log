...
struct TaskLocalLoggerTest {
    ...
    @Test func withLoggerProvidesDefaultLogger() {
        // Test that withLogger provides a fallback logger when no context is set
        withLogger { logger in
            #expect(logger.label == "task-local-fallback")
        }
    }

    @Test func withLoggerSyncVoid() {
        let logging = TestLogging()
        let logger = Logger(label: "test", factory: { logging.make(label: $0) })

        withLogger(logger) { _ in
            withLogger(mergingMetadata: ["test": "value"]) { logger in
    ...
    @Test func withLoggerSyncReturning() {
        let logging = TestLogging()
        let logger = Logger(label: "test", factory: { logging.make(label: $0) })

        let result = withLogger(logger) { logger in
    ...
    @Test func withLoggerAsyncVoid() async {
        let logging = TestLogging()
        let logger = Logger(label: "test", factory: { logging.make(label: $0) })

        await withLogger(logger) { _ in
            await withLogger(mergingMetadata: ["test": "async"]) { logger in
    ...
    @Test func withLoggerAsyncReturning() async {
        let logging = TestLogging()
        let logger = Logger(label: "test", factory: { logging.make(label: $0) })

        let result = await withLogger(logger) { _ in
            await withLogger(mergingMetadata: ["test": "async"]) { logger in
    ...
    // MARK: - withLogger() with modifications

    @Test func withLoggerMetadataSyncVoid() {
        let logging = TestLogging()
        let logger = Logger(label: "test", factory: { logging.make(label: $0) })
    ...
    @Test func withLoggerMetadataSyncReturning() {
        let logging = TestLogging()
        let logger = Logger(label: "test", factory: { logging.make(label: $0) })

        let result = withLogger(logger) { _ in
            withLogger(mergingMetadata: ["key": "value"]) { logger in
                logger.info("computing")
                return 100
            }
        ...
        #expect(result == 100)
        logging.history.assertExist(level: .info, message: "computing", metadata: ["key": "value"])
    }

    @Test func withLoggerMetadataAsyncVoid() async {
        let logging = TestLogging()
        let logger = Logger(label: "test", factory: { logging.make(label: $0) })

        await withLogger(logger) { _ in
            await withLogger(mergingMetadata: ["async": "true"]) { logger in
    ...
    @Test func withLoggerMetadataAsyncReturning() async {
        let logging = TestLogging()
        let logger = Logger(label: "test", factory: { logging.make(label: $0) })
    ...
    @Test func withLoggerHandler() {
        ...
        withLogger(logger) { _ in
            withLogger(Logger(label: "custom", customHandler)) { logger in
    ...
    @Test func withLoggerMetadataAndLogLevel() {
        let logging = TestLogging()
        let logger = Logger(label: "test", factory: { logging.make(label: $0) })
    ...
    @Test func nestedWithLoggerAccumulatesMetadata() {
        let logging = TestLogging()
        let logger = Logger(label: "test", factory: { logging.make(label: $0) })

        withLogger(logger) { _ in
            withLogger(mergingMetadata: ["level1": "first"]) { logger in
                logger.info("level 1")
    ...
    @Test func nestedMetadataOverrides() {
        ...
        withLogger(logger) { _ in
            withLogger(mergingMetadata: ["key": "original"]) { _ in
                withLogger(mergingMetadata: ["key": "override"]) { logger in
                    logger.info("test")
                }
            }
        }
        ...
        await withLogger(logger) { _ in
            await withLogger(mergingMetadata: ["context": "parent"]) { _ in
                await withTaskGroup(of: Void.self) { group in
                    ...
                    // Task 3
                    group.addTask {
                        Logger.current.info("task 3 message")
                    }
                }
            }
        }
    ...
    @Test func childTaskInheritsParentContext() async {
        ...
        await withLogger(logger) { _ in
            await withLogger(mergingMetadata: ["parent": "value"]) { logger in
                logger.info("parent message")

                // Create child task — inherits task-local context
                await Task {
                    Logger.current.info("child message")
                }.value
            }
        }
    ...
    @Test func detachedTaskDoesNotInheritContext() async {
        let logging = TestLogging()
        let logger = Logger(label: "test", factory: { logging.make(label: $0) })
        ...
        await withLogger(logger) { _ in
            await withLogger(mergingMetadata: ["parent": "value"]) { logger in
                ...
                // Detached task does NOT inherit task-local context
                await Task.detached {
                    Logger.current.info("detached message")
                }.value
            }
        }
        ...
        // Detached task should NOT have the parent metadata — it uses the fallback logger,
        // so the message does not appear in our test logging history at all.
        logging.history.assertNotExist(level: .info, message: "detached message")
    }
    ...
    @Test func childTaskCanOverrideContext() async {
        ...
        await withLogger(logger) { _ in
            await withLogger(mergingMetadata: ["parent": "original"]) { logger in
                logger.info("parent")

                // Child overrides context
                await Task {
                    withLogger(mergingMetadata: ["parent": "overridden"]) { logger in
                        logger.info("child")
                    }
                }.value
    ...
    @Test func contextPreservedThroughAsyncFunctions() async {
        ...
        func innerAsync() async {
            Logger.current.info("inner function")
        }

        func outerAsync() async {
            Logger.current.info("outer before")
            await innerAsync()
            Logger.current.info("outer after")
        }

        await withLogger(logger) { _ in
            await withLogger(mergingMetadata: ["flow": "async"]) { _ in
                await outerAsync()
            }
        }
    ...
    @Test func logLevelCanBeChanged() {
        ...
        withLogger(logger) { _ in
            withLogger(logLevel: .error) { logger in
                logger.info("first - should not appear")

                withLogger(logLevel: .info) { logger in
    ...
    @Test func withHandler() {
        ...
        // Create a new logger with a different handler
    ...
    @Test func requestHandlerPattern() async {
        ...
        func processRequest(id: String) async {
            await withLogger(mergingMetadata: ["request.id": "\(id)"]) { logger in
        ...
        func authenticateUser(username: String) async {
            withLogger(mergingMetadata: ["user": "\(username)"]) { logger in
    ...
    @Test func libraryEntryPointPattern() {
        ...
        struct DatabaseClient {
            func query(_ sql: String) {
                Logger.current.debug("Executing query", metadata: ["sql": "\(sql)"])
            }
        }
    ...
    // MARK: - Throwing closure propagation

    @Test func withLoggerSyncPropagatesThrow() {
    ...
    @Test func withLoggerAsyncPropagatesThrow() async {
        struct TestError: Error, Equatable {}
        let logging = TestLogging()
        let logger = Logger(label: "test", factory: { logging.make(label: $0) })
    ...
    @Test func withLoggerModifySyncPropagatesThrow() {
        struct TestError: Error, Equatable {}
        let logging = TestLogging()
        let logger = Logger(label: "test", factory: { logging.make(label: $0) })
        ...
        #expect(throws: TestError.self) {
            try withLogger(logger) { _ in
                try withLogger(mergingMetadata: ["key": "value"]) { _ in
                    throw TestError()
                }
            }
        }
    ...
    @Test func withLoggerModifyAsyncPropagatesThrow() async {
        struct TestError: Error, Equatable {}
        let logging = TestLogging()
        let logger = Logger(label: "test", factory: { logging.make(label: $0) })
        ...
        await #expect(throws: TestError.self) {
            try await withLogger(logger) { _ in
                try await withLogger(mergingMetadata: ["key": "value"]) { _ in
                    throw TestError()
                }
            }
        }
    ...
    // MARK: - Fallback with modifications

    @Test func withLoggerModifyWithoutPriorContext() {
        // When no task-local logger is set, withLogger(mergingMetadata:) should use
        // the fallback logger and still merge metadata correctly.
        withLogger(mergingMetadata: ["key": "value"]) { logger in
            #expect(logger.label == "task-local-fallback")
            #expect(logger.handler.metadata["key"] == "value")
        }
    }

    // MARK: - MetadataProvider interaction

    @Test func withLoggerMetadataProvider() {
        let logging = TestLogging()
        let logger = Logger(label: "test", factory: { logging.make(label: $0) })
        ...
        withLogger(logger) { _ in
            withLogger(metadataProvider: provider) { logger in
