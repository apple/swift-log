...
struct RedactionTests {
    ...
    func testSensitivityAttributeStrippedAfterRedaction() {
        ...
        #expect(finalMetadata?["user.id"]?.value.description == TestData.redactionMarker)
        #expect(finalMetadata?["user.id"]?.attributes.sensitivity == nil)

        // The public value should retain its sensitivity attribute
        #expect(finalMetadata?["action"]?.value.description == TestData.action)
        #expect(finalMetadata?["action"]?.attributes.sensitivity == .public)
    }
    ...
    func testMetadataProviderPlainAndAttributed() {
        // Plain provider: get() returns values, getAttributedMetadata() wraps with empty attributes
        ...
        let plainAttributed = plainProvider.getAttributedMetadata()
        ...
        // Attributed provider: getAttributedMetadata() returns full metadata, get() strips attributes
        let attributedProvider = Logger.MetadataProvider.attributed {
            [
        ...
        let attributedMetadata = attributedProvider.getAttributedMetadata()
    ...
    func testMetadataProviderMultiplexWithAttributedProviders() {
        ...
        // getAttributedMetadata() should return all metadata with attributes preserved
        let attributed = multiplexed!.getAttributedMetadata()
    ...
    func testMetadataProviderMultiplexAttributedLastWriterWins() {
        let provider1 = Logger.MetadataProvider.attributed {
            [
                "key": "\("from-first", sensitivity: .public)"
            ]
        }
        let provider2 = Logger.MetadataProvider.attributed {
            [
    ...
    func testMetadataProviderMultiplexAttributedWithEmptyProvider() {
        let emptyAttributed = Logger.MetadataProvider.attributed {
...
internal struct RedactionTestLogHandler: LogHandler {
    ...
    subscript(metadataKey metadataKey: String) -> Logger.Metadata.Value? {
        get { self.attributedMetadata[metadataKey]?.value }
        set {
            if let newValue {
                self.attributedMetadata[metadataKey] = .init(newValue, attributes: .init())
            } else {
                self.attributedMetadata[metadataKey] = nil
            }
        }
    }
