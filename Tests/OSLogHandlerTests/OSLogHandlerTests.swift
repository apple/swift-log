...
struct OSLogHandlerTests {
    ...
    @Test("OSLogHandler logs attributed metadata with visibility")
    func testAttributedMetadataWithVisibility() {
        guard #available(macOS 11.0, iOS 14.0, tvOS 14.0, watchOS 7.0, *) else {
            return
        }
        ...
        // Should not crash - redaction is handled by OSLog
        logger.info(
            "User action",
            attributedMetadata: [
    ...
    func testHandlerAttributedMetadataSubscript() {
        ...
        handler[attributedMetadataKey: "user.id"] = "\(TestData.userId, sensitivity: .sensitive)"
        #expect(handler[attributedMetadataKey: "user.id"]?.attributes.sensitivity == .sensitive)
    ...
    func testHandlerAttributedMetadataProperty() {
        ...
        #expect(handler.attributedMetadata["public-key"]?.attributes.sensitivity == .public)
        #expect(handler.attributedMetadata["private-key"]?.attributes.sensitivity == .sensitive)
    }
}
#endif
